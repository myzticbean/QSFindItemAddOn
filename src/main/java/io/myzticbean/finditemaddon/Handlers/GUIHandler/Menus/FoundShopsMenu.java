...
public class FoundShopsMenu extends PaginatedMenu {
    ...
    private final NamespacedKey key = new NamespacedKey(FindItemAddOn.getInstance(), "locationData");
    private final NamespacedKey nearestWarpKey = new NamespacedKey(FindItemAddOn.getInstance(), "nearestWarp");
                ...
                if (FindItemAddOn.getConfigProvider().TP_PLAYER_DIRECTLY_TO_SHOP) {
                    if (playerMenuUtility.getOwner().hasPermission(PlayerPermsEnum.FINDITEM_SHOPTP.value())) {
                        ...
                        if (locToTeleport != null) {
                            // Check if player is banned from the warp
                            if (PlayerWarpsPlugin.getIsEnabled()) {
                                String warp = meta.getPersistentDataContainer().get(nearestWarpKey, PersistentDataType.STRING);
                                if (warp != null && !StringUtils.isEmpty(warp) && PlayerWarpsUtil.isPlayerBanned(warp, player))
                                    return;
                ...
                if (foundShops.get(index) != null) {
                    ...
                    if (FindItemAddOn.getConfigProvider().TP_PLAYER_DIRECTLY_TO_SHOP) {
                        ...
                        // if Nearest Warp is set to PlayerWarps, store the warp name
                            ...
                            meta.getPersistentDataContainer().set(nearestWarpKey, PersistentDataType.STRING, nearestPlayerWarp.getWarpName());
                        }
                    }
